...
<script type="text/javascript" src="${webRoot}/js/jquery-1.8.3.js"></script>
<script>
	...
	function showOnLine() {
		// 异步发送请求 获取在线人员列表
		// Jquery发送异步请求
		$.post("${webRoot}/online.jsp?" + new Date().getTime(), function(data) {
	...
	function showContent(){
		$.post("${webRoot}/userController.do?getMessage&"+new Date().getTime(),function(data){
			$("#content").html(sysBBS+data);
		});
	}
	function set(name) {
		if (name != '${existUser.username}') {
			form1.to.value = name;
		} else {
			alert("不能选择自己");
		}
	...
	function send() {
		...
		$.post("${webRoot}/userController.do?sendMessage&"+new Date().getTime(),$("#form1").serialize(),function(data){
	...
	function exit() {
		alert("欢迎下次光临");
		window.location.href = "${webRoot}/userController.do?exit";
	}
	...
	function check() {
		$.post("${webRoot}/userController.do?check",function(data){
			if(data==1){
				alert("请重新登录！");
